...
values ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '11111111-1111-1111-1111-111111111111', 'A');
select is((select count(*)::int from public.projects), 1, 'owner can read own project');
select throws_ok(
  $$update public.projects set user_id = '22222222-2222-2222-2222-222222222222' where id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'$$,
  '42501',
  null,
...
select throws_ok(
  $$insert into public.projects (user_id, name) values ('11111111-1111-1111-1111-111111111111', 'forged')$$,
  '42501',
  null,
  'cross-user project insert denied'
);
select throws_ok(
  $$insert into public.assets (project_id, user_id, kind, bucket, object_path, mime_type, byte_size)
    values ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '22222222-2222-2222-2222-222222222222', 'source_front', 'product-uploads', '22222222-2222-2222-2222-222222222222/x.png', 'image/png', 12)$$,
  '42501',
  null,
